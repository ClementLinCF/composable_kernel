...
struct GridGemmV1
{
                               ...
                               const CElementFunction& c_element_func) const
    {
        ...
        // divide problem
        const auto id_block = ck_tile::get_block_id();
        ...
        const auto iM = __builtin_amdgcn_readfirstlane(id_tile.template At(Number<0>{}) * kMPerBlock);
        const auto iN = __builtin_amdgcn_readfirstlane(id_tile.template At(Number<1>{}) * kNPerBlock);

        // A block window
        auto a_block_window = ck_tile::make_tile_window(
            a_grid, ck_tile::make_tuple(ck_tile::number<kMPerBlock>{}, ck_tile::number<kKPerBlock>{}), {iM, 0});

        // B block window
        auto b_block_window = ck_tile::make_tile_window(
            b_grid, ck_tile::make_tuple(ck_tile::number<kNPerBlock>{}, ck_tile::number<kKPerBlock>{}), {iN, 0});
        ...
        // store C
        auto c_window = ck_tile::make_tile_window(
            c_grid, ck_tile::make_tuple(ck_tile::number<kMPerBlock>{}, ck_tile::number<kNPerBlock>{}), {iM, iN});
