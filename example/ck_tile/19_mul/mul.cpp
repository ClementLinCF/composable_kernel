...
auto create_args(int argc, char* argv[])
{
    ...
    arg_parser.insert("m", "1", "m dimension")
...
bool run(const ck_tile::ArgParser& arg_parser)
{
    ...
    [[maybe_unused]] int do_validation  = arg_parser.get_int("v");
    ...
    using Porblem =
        ck_tile::MulProblem<XDataType, ComputeDataType, YDataType, Shape, ReduceOp>;

    // using Kernel = ck_tile::Reduce<Porblem>;
    using Kernel = ck_tile::Mul<Porblem>;
    ...
    if(do_validation)
    {
        ...
        ck_tile::reference_mul<XDataType, YDataType>(
