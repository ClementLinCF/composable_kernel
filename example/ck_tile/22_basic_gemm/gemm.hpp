...
struct Gemm
{
    ...
    struct GridGemmPolicy
    {
        ...
        CK_TILE_HOST_DEVICE static constexpr auto MakeBlock2TileMap(ck::index_t NumTilesM,
                                                                    ck::index_t NumTilesN)
        {
            const auto unmerge = ck::make_merge_transform(ck::make_tuple(NumTilesN, NumTilesM));

            return [unmerge](index_t block_id) {
                ck::MultiIndex<2> unmerged;
                unmerge.CalculateLowerIndex(unmerged, ck::make_multi_index(block_id));

                return make_multi_index(unmerged.At(ck::Number<1>{}), unmerged.At(ck::Number<0>{}));  
